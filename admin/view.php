...
<?php function today() { ?>
        ...
        <div class = "menu_date">
            <img src="download.png" alt="GhanaDotCom" id = "pic">
        ...
        <form method = "POST" action = "">
            <div class="d-grid gap-2 col-6 mx-auto">
                <div class="text-center">
                    <a href="#sending" data-toggle="modal" class="btn btn-flat btn-primary mr-3 mb-3"><i class='bx bx-send'></i><span>Sender Order</span></a>
                    <button type = "submit" name = "today" value = "today" class="btn btn-flat btn-primary mr-3 mb-3"><i class='bx bx-show'></i><span>Today's Orders</span></button>
                    <button type = "submit" name = "previous"  value = "previous" class="btn btn-flat btn-primary mr-3 mb-3"><i class='bx bxs-file-find' ></i><span>Previous Orders</span></button>
                </div>
            </div>
        </form>
        <?php } ?>
            ...
            <?php function previous (){ ?>
                <div class="menu">
        <div class="heading">
            <h3>&mdash; Previous Orders &mdash; </h3>
            <div class = "log">
                <a href="logout.php"><i class='bx bx-log-out-circle'></i><span>logout</span></a>
            </div>
        </div>
        ...
        <diV class = "container mb-3">
            ...
            <tbody>
                <?php
                    ...
                    $today = date("Y-m-d");
                    $sql1 = "SELECT `order`.`id`, `order`.`date`, `firstname`, `lastname`, `email`, `food`, `time` FROM `order` LEFT JOIN `users` ON `users`.`id` = `order`.`user_id` LEFT JOIN `dish` ON `dish`.`id` = `order`.`food_id` WHERE 1"; //`date` = '".$today."'";
                    $result1 = mysqli_query($conn, $sql1);
                    $queryResult = mysqli_num_rows($result1);
                    $count = 1;
                    if($queryResult > 0){
                        while ($check = mysqli_fetch_assoc($result1)){
                            ...
                            <tr>
                                ...
                                <td>".$check['food']."</td>
                                <td>".$check['date']."</td>
                                <td>".date("h:i A", strtotime($check['time']))."</td>
                            </tr>";
                            $count++;
                        }    
                    }
